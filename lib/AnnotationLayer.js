...
import {
  ...
  getFilenameFromUrl,
  LinkTarget
} from "pdfjs-dist/lib/display/dom_utils";
import {
...
class AnnotationElement {
  constructor(parameters, isRenderable = false, ignoreBorder = false) {
    ...
    this.annotationObject = parameters.annotationObject;
  ...
  _createContainer(ignoreBorder = false) {
    ...
    if (!ignoreBorder && data.borderStyle.width > 0) {
      ...
      if (!this.renderInteractiveForms) container.style.border = "none";
  ...
  render() {
    ...
    if (this.renderInteractiveForms) {
      ...
      if (this.data.multiLine) {
        ...
        element.setAttribute("id", this.data.fieldName);
        element.setAttribute("data-id", this.data.fieldName);
      } else {
        ...
        element.addEventListener("keydown", event => {
          checkIfCanWriteMore(event.target);
          getBestFontSize(event.target, this.container);
        });
        element.addEventListener("keyup", event => {
          this.annotationObject.saveData(event.target);
        });
      }
      ...
      this.container.style.height =
        parseFloat(this.container.style.height.replace("px", "")) + 2 + "px";
    ...
    onReady(element, () => {
      getBestFontSize(element, this.container);
    });
...
class CheckboxWidgetAnnotationElement extends WidgetAnnotationElement {
  ...
  render() {
    ...
    let element;
    this.data.fieldValue = this.data.fieldValue === true ? "checked" : "Off";
    element = document.createElement("input");
    element.disabled = this.data.readOnly;
    ...
    if (this.renderInteractiveForms) {
      element.addEventListener("click", event => {
        this.annotationObject.saveData({
          id: event.target.id,
...
class AnnotationLayer {
  ...
  static render(parameters) {
    for (let i = 0, ii = parameters.annotations.length; i < ii; i++) {
      ...
      data.fieldValue =
        parameters.annotationObject.annotations[data.fieldName] ||
        parameters.annotations[i].fieldValue;
      if (!data) {
        continue;
      }
      let element = AnnotationElementFactory.create({
        ...
        annotationObject: parameters.annotationObject
      });
...
const getBestFontSize = (element, container) => {
  var defaultFontSize = 8;
  var fontSize = defaultFontSize;
  let borderWidth =
    parseInt(container.style.borderWidth.replace("px", "")) || 4;
  do {
    element.setAttribute("style", "font-size: " + fontSize + "px !important");
    fontSize--;
  } while (
    element.scrollWidth > element.offsetWidth + borderWidth &&
    fontSize > 4
  );
};

const onReady = (element, callback) => {
  element.setAttribute(
    "style",
    "visibility: hidden; " + element.getAttribute("style")
  );
  let intervalId = setInterval(() => {
    if (element.scrollWidth > 0 && element.offsetWidth > 0) {
      callback();
      element.setAttribute(
        "style",
        "visibility: visible; " + element.getAttribute("style")
      );
...
const checkIfCanWriteMore = element => {
  let fontSize = parseInt(element.style.fontSize.replace("px", ""));
  if (fontSize === 5 && element.scrollWidth > element.offsetWidth - 2) {
    element.maxLength = element.value.length - 1;
